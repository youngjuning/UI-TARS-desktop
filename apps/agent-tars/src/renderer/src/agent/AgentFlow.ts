...
export class AgentFlow {
  ...
  async run() {
    ...
    globalEventEmitter.addListener(this.appContext.agentFlowId, async event => {
    ...
    try {
      while (!this.abortController.signal.aborted && !this.hasFinished) {
        ...
        try {
          ...
          // Check if all tasks are done after normalizing the plan
          if (
            awareResult.plan &&
            agentContext.plan.every(task => task.status === PlanTaskStatus.Done)
          ) {
            this.hasFinished = true;
            break;
          }
